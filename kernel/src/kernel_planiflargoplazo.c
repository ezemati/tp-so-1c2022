...
{
    estado_proceso estado_anterior = proceso_cambiar_estado(pcb, S_EXIT);

    log_info_with_mutex(logger, &mutex_logger, "Pasando proceso %d de %s a EXIT", pcb->id, estado_proceso_to_string(estado_anterior));
