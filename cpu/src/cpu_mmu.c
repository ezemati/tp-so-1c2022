...
uint32_t traducir_direccion_logica_a_fisica(uint32_t direccion_logica)
{
    ...
    if (numero_marco != -1)
    {
        ...
        log_info_with_mutex(logger, &mutex_logger, "TLB HIT: {numero_pagina=%d, numero_marco=%d}", numero_pagina, numero_marco);
    }
    else
    {
        ...
        log_info_with_mutex(logger, &mutex_logger, "TLB MISS: la pagina %d no esta en la TLB", numero_pagina);
        ...
        if (numero_pagina_reemplazada != -1)
        {
            ...
            log_info_with_mutex(logger, &mutex_logger, "TLB: entrada {numero_pagina=%d, numero_marco=%d} agregada (reemplazando entrada con numero_pagina=%d)", numero_pagina, numero_marco, numero_pagina_reemplazada);
        }
        else
        {
    ...
    log_info_with_mutex(logger, &mutex_logger, "MMU: {direccion_logica=%d, direccion_fisica=%d}", direccion_logica, direccion_fisica);
